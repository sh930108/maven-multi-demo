...
jQuery.extend(qBox,{
    aDgs:[],
    iFrame:function(op){ 
            op = jQuery.extend({title:'提示',w:400,h:300,src:'about:blank',closeText:"关闭",closeable:true,modal:true,fixed:true,unloadOnHide:true},op),fm=parseInt(Math.random()*(1000*987));//
            var dialog = new Boxy("<b id=\"ld"+fm+"\">正在加载，请稍后....</b><iframe id=\"_"+fm+"\" style=\"width:0px;height:0px;display:none;padding:0;\" src="+op.src+" frameborder=\"0\" scrolling=\"auto\"></iframe>",op);   
    ...
    dialog: function(element, op) {
    	op = jQuery.extend({title:'提示',w:400,h:300,src:'about:blank',closeText:"关闭",closeable:true,modal:true,fixed:true,unloadOnHide:true},op),fm=parseInt(Math.random()*(1000*987));//
        var dialog = new Boxy(element, op);   
